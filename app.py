...
path = 'Fire_Module/yolov5/best.pt'
model = torch.hub.load('Fire_Module/yolov5', 'custom', path, source='local', force_reload=True)
# Set Model Settings
model.eval()
...
with mp_pose.Pose(
    ...
    out = cv2.VideoWriter("Fall_Module/results/output.avi", cv2.VideoWriter_fourcc(*"MJPG"), 20, size)
